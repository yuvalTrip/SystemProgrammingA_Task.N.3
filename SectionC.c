...
char txt[TXT + 1];
int AlreadyBeen[sizeof(word)];//define new array for word , that contain 1 if we already check this letter and 0 if we not been there before
...
void Fun3()
{
        ...
        char sAns[sizeof(txt)];//define new array in size of txt (in max)
        ...
        for (int i = 0; i < sizeof(txt); ++i) 
        {
                ...
                if(contain(word,ch_temp))
                {
                    ...
                    for (k = 0; k < sizeof(word); ++k)
                    {
                        ...
                        if(i+Counter<sizeof(txt)&&contain(word,txt[j+Counter]))
                        {
                            sAns[Counter]=txt[j+Counter];
                            Counter++;
                        }
                        else
                        {
